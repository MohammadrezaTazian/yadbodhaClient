...
export default function FamilyTreePersonModal(props: PropsModel) {
  ...
  useEffect(() => {
    if (props.familyTreePerson) {
      if (props.familyTreePerson.familyTreePersonId === 0) {
        initForm();
      } else {
  ...
  const initForm = () => {
    props.modalOpen && form.setFieldValue('isDeceased', 0);
  ...
  const onOk = async () => {
    try {
      ...
      if (props.familyTreePerson && props.familyTreePerson.familyTreeId) {
        ...
        setIsLoading(true);
        props.familyTreePerson.familyTreePersonId === 0
          ? await familyTreeServices.familyTreePersonAdd(formData)
          : await familyTreeServices.familyTreePersonUpdate(formData, props.familyTreePerson.familyTreePersonId!);
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      {contextHolder}
      <Modal
        title={props.familyTreePerson && props.familyTreePerson.familyTreeId === 0 ? 'تعریف فرد در شجره نامه' : 'ویرایش فرد در شجره نامه'}
        centered
          ...
          <Form form={form} layout="vertical" className="form-width">
            ...
            <Row gutter={24}>
              <Col xs={24} sm={12} span={12}>
                <Form.Item name="familyTreePersonName" label="نام و نام خانوادگی" rules={[{ required: true, message: 'لطفاً نام را وارد کنید!' }]}>
